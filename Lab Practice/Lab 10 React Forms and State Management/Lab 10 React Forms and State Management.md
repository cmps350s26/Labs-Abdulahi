...
No `e.preventDefault()`. No `useState` per field. No `fetch()`. The `name` attribute on each input is what matters — `Object.fromEntries(formData)` converts all form fields into a plain object like `{ description: "Groceries", amount: "850", type: "expense", ... }`.
...
- Wire delete buttons on list pages and budget cards
...
- Delete resources using a server action called from a client component
...
1. **`"use server"`** at the top marks every exported function as a server action. It runs on the server, never ships to the browser.
2. **`Object.fromEntries(formData)`** converts all form fields into a plain object. Each input's `name` attribute becomes a key.
3. **`Number(data.amount)`** converts the string to a number — form inputs always give strings.
4. **`revalidatePath` + `redirect`** refreshes the page data and navigates back to the list.
...
// TODO 1c: await transactionsRepo.create(data)
// TODO 1d: revalidatePath("/transactions") then redirect("/transactions")
```
...
`const { id, ...fields } = Object.fromEntries(formData)` separates the ID from the rest. The `...fields` spread collects everything except `id` into a new object. The `id` comes from `<input type="hidden" name="id">` in the form.
...
// TODO 2c: await transactionsRepo.update(id, fields)
...
On the transactions list page, the Delete button and `handleDelete` function wire it together:
...
// Delete button in each row (already provided):
...
The form page reads the URL with `useSearchParams` to detect add vs edit:
...
The form uses `action={action}` to wire the server action, and `defaultValue={transaction?.description || ""}` to pre-fill fields in edit mode.
...
```jsx
// TODO 5: createBudgetAction — Object.fromEntries, coerce budgeted/spent/year to Number, create, revalidate, redirect
// TODO 6: updateBudgetAction — destructure { id, ...fields }, coerce, update, revalidate, redirect
...
// TODO 10d: Pass action to the <form> tag: action={action}
// TODO 10e: Change title to {isEdit ? "Edit" : "Add"} Budget
```
